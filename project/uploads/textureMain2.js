...
function setUpTextures(){
    ...
    myImage.src = 'mytexture.jpg'; 
    myImage.onload = () => { doLoad (myImageTexture, myImage); };
}
...
function drawCurrentShape () {
    ...
    let mode = 0;
    if (curTexture === "myimage") mode = 1;
    if (curTexture === "proc") mode = 2;
    gl.uniform1i(program.uTexMode, mode);
    ...
    gl.uniform1i (program.uMyImage, 1);
...
function initProgram (vertexid, fragmentid) {
  ...
  program.uMyImage = gl.getUniformLocation(program, 'uMyImage');
  program.uTexMode = gl.getUniformLocation(program, 'uTexMode');
